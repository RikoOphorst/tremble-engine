...
namespace tremble
{
	std::unordered_map<std::string, GraphicsPSO> GraphicsPSO::psos_ = std::unordered_map<std::string, GraphicsPSO>();
	std::unordered_map<std::string, ComputePSO> ComputePSO::psos_ = std::unordered_map<std::string, ComputePSO>();
	...
	//------------------------------------------------------------------------------------------------------
	GraphicsPSO& GraphicsPSO::Get(const std::string& name)
	{
		auto& find = psos_.find(name);

		if (find == psos_.end())
		{
			psos_.emplace(std::make_pair(name, GraphicsPSO()));
		}
	...
	void GraphicsPSO::Finalize()
	{
		...
		CHECKHR(Get::Device().Get()->CreateGraphicsPipelineState(&pso_desc_, IID_PPV_ARGS(&pso_)));
	}
	...
	//------------------------------------------------------------------------------------------------------
	ComputePSO& ComputePSO::Get(const std::string& name)
	{
		auto& find = psos_.find(name);

		if (find == psos_.end())
		{
			psos_.emplace(std::make_pair(name, ComputePSO()));
		}
