...
namespace tremble
{
	...
	public:
		...
		void CreateFromSwapChain(const std::wstring& name, ID3D12Resource* swap_chain_resource);

		void CreateFromTexture(ID3D12Resource* texture_resource, int slice = 0);

		void Create(const std::wstring& name, UINT width, UINT height, DXGI_FORMAT format);
