...
{
	//------------------------------------------------------------------------------------------------------
	void UploadBuffer::Create(const std::wstring& name, UINT num_elements, UINT element_size, void* initial_data)
	{
	...
	void UploadBuffer::Map()
	{
		if (!is_mapped_)
		{
			CHECKHR(resource_->Map(0, nullptr, reinterpret_cast<void**>(&mapped_data_)));
			is_mapped_ = true;
		}
	}
