...
namespace tremble
{
	...
	//------------------------------------------------------------------------------------------------------
	void GpuBuffer::Create(const std::wstring& name, UINT num_elements, UINT element_size, void* initial_data, bool create_views)
	{
		...
		CHECKHR(Get::Renderer()->GetDevice().Get()->CreateCommittedResource(&heap_properties, D3D12_HEAP_FLAG_NONE, &resource_desc, usage_state_, nullptr, IID_PPV_ARGS(&resource_)));
