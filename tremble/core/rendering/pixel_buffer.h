...
namespace tremble
{
	...
	protected:
		...
		void AssociateWithResource(const std::wstring& name, ID3D12Resource* resource, const D3D12_RESOURCE_STATES& current_resource_state);
		void CreateTextureResource(const std::wstring& name, const D3D12_RESOURCE_DESC& resource_desc, const D3D12_CLEAR_VALUE& clear_value);
