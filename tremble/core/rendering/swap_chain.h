...
namespace tremble
{
	...
	public:
		...
		ColorBuffer& GetBuffer(UINT buffer_index) { assert(buffer_index < buffer_count_); return buffers_[buffer_index]; }
		const ColorBuffer& GetBuffer(UINT buffer_index) const { assert(buffer_index < buffer_count_); return buffers_[buffer_index]; }
