...
namespace tremble
{
		...
		type_(type)
	{
		...
		CHECKHR(device->CreateCommandQueue(&queue_desc, IID_PPV_ARGS(&queue_)));
		queue_->SetName(L"CommandQueue");

		CHECKHR(device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fence_)));
	...
	{
		CHECKHR(((ID3D12GraphicsCommandList*)list)->Close());
