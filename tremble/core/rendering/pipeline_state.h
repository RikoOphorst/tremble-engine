...
namespace tremble
{
	...
	public:
		static GraphicsPSO& Get(const std::string& name);
	...
	protected:
		static std::unordered_map<std::string, GraphicsPSO> psos_;
	...
	public:
		static ComputePSO& Get(const std::string& name);
		...
		void Finalize() override;
	protected:
		static std::unordered_map<std::string, ComputePSO> psos_;
