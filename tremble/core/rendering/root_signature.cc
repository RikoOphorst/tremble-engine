...
namespace tremble
{
	...
	void RootSignature::Finalize(const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS flags)
	{
		...
		//CHECKHR(Get::Device().Get()->CreateRootSignature(0, output_blob->GetBufferPointer(), output_blob->GetBufferSize(), IID_PPV_ARGS(&root_signature_)));
		CHECKHR(Get::Renderer()->GetDevice().Get()->CreateRootSignature(0, output_blob->GetBufferPointer(), output_blob->GetBufferSize(), IID_PPV_ARGS(&root_signature_)));
